...
This generated workflow is the shared source of truth for mandatory and optional AI-SDLC initialization steps in `oh-my-githubcopilot` (topology: standalone, depth 0).
...
- **skill-modernization** - enabled when the target repo owns a skill catalog; status: `not-applicable-not-a-catalog` (this is a GitHub Copilot CLI plugin, not a skills catalog).

## Entry surface links

Generated `AGENTS.md` and `README.md` must link to this workflow doc and `.ai/workflows/repo-workflow.json`. `CLAUDE.md` and `GEMINI.md` are thin pointers to `AGENTS.md` (ADR-0007) and are not entry surfaces.
