 ...
 *
 * ID:       tdd
 * Keywords: /omp:tdd, tdd:
 * Tier:     development workflow
 *
...
export async function activate(input: SkillInput): Promise<SkillOutput> {
  ...
  const baseArgs = ["bin/omp.mjs", "tdd", ...input.args];
  return new Promise((resolve) => {
    const child = spawn("node", baseArgs, { stdio: "inherit" });
    child.on("close", (code) => {
      resolve({ status: code === 0 ? "ok" : "error", message: `tdd exited with code ${code}` });
    });
